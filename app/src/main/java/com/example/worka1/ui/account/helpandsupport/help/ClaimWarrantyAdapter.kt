...
class ClaimWarrantyAdapter(
    private val name: List<String>,
    private val navController: NavController
) : RecyclerView.Adapter<ClaimWarrantyAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener {
            when (position) {
                0 -> navController.navigate(R.id.navigation_claim_warranty)
                1 -> navController.navigate(R.id.navigation_claim_warranty)
            }
        }
    }
